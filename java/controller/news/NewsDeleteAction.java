...
public class NewsDeleteAction implements Action {
    ...
    public ActionForward execute(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (memberId == null) {
            forward.setPath("loginPage.do");   // 너 프로젝트가 loginPage.do 라면
        ...
        // 3) 뉴스 PK / 유저 ID / 컨디션 담기(딜리트 뉴스)
        NewsDTO newsDTO = new NewsDTO();
        newsDTO.setNewsId(newsId);
        newsDTO.setCondition("NEWS_DELETE");

        // 4) 삭제 메서드 호출(M)  (보통 소프트삭제: CONDITION 업데이트)
        NewsDAO newsDAO = new NewsDAO();
        boolean deleteNews = newsDAO.delete(newsDTO);

        if (deleteNews) {
            // 성공 → 뉴스 목록으로
            forward.setPath("newsList.do"); // 뉴스 전체 페이지(목록) URL로 교체
            forward.setRedirect(true);
        } else {

...
/* (옵션) 썸네일 되돌리기 버튼 - 작성페이지 UX 해치지 않게 최소 스타일 */
.thumb-reset-btn{
  background:#2b2b3c;
  color:#fff;
  padding:8px 14px;
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  /* ===============================
     1) 수정페이지 진입 시, 본문 이미지 src 먼저 보정
        (에디터 생성 전에 textarea 값 정리)
  =============================== */
  ...
  // 4-3) 되돌리기: 파일 선택 취소 + 기존 썸네일로 복원(기존이 있으면)
  ...
  /* ===============================
     5) 관련 애니 검색 (작성페이지 UX 그대로 + 키 불일치 방어)
  =============================== */
  ...
  if (input && resultBox) {
    ...
    input.addEventListener('input', function () {
      ...
      // 사용자가 타이핑을 시작하면 “선택된 animeId”는 무효 처리

...
<style>
/* =========================
   News Detail (Board Detail Glass Tone)
   - ✅ 형식/구조 변경 없음 (HTML/JSTL/JS 그대로)
   - ✅ 게시글 상세보기(bd-card) 톤으로만 디자인 변환
   - ✅ 본문 이미지 "직각" (라운드 제거)
========================= */

/* (선택) 페이지 배경을 조금 더 '게시글 상세' 느낌으로 */
.blog-details.spad{
...
.news-crumb{
  ...
  gap:8px;
  font-size:14px;
  font-weight: 500;         /* (선택) 더 또렷하게 */
  letter-spacing: .2px;     /* (선택) UI 느낌 */
...
#newsContent.news-prose{
  padding: 0 10px;   /* ← 10~18px 사이로 취향 조절 */
  max-width: 920px;
  margin: 0 auto;
...
/* ✅ "본문 위는 밝은데 아래는 희미" 방지:
   CKEditor가 span에 inline color를 박아두는 경우가 많아서
   문단(p) 내부 텍스트는 강제로 같은 톤으로 통일 */
...
#newsContent.news-prose p:last-child{
  margin-bottom: 10px !important; /* ✅ 관련애니와 숨 쉬는 공간 */
}
